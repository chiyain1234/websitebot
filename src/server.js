...
module.exports = async (client) => {
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://discordbotlist.lmgxenon.repl.co/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://discordbotlist.lmgxenon.repl.co/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://discordbotlist.lmgxenon.repl.co/bot/" + bot.botID + "</loc></url>";
        })
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://discordbotlist.lmgxenon.repl.co/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
            .addField("Code Link", `https://discordbotlist.lmgxenon.repl.co/code/${kod}`, true)
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://discordbotlist.lmgxenon.repl.co/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addField("Code Link", `https://discordbotlist.lmgxenon.repl.co/code/${kod}`, true)
            .addField("Code Description", rBody['codedesc'], true)
            .addField("Code Category", rBody['category'], true)
        )
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("Topic.xyz", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`<a:online:833375738785824788> Topic are **active** again!\n[Click to redirect website](https://topic.xyz)`)
        ...
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ <a:maintence:833375738547535913> Topic has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
